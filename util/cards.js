...
class Cards {
    ...
    getQueryURL() {
        ...
        if (!STEAM_API_KEY) return console.error('Steam API key is undefined');

        const joinedIds = this.cards.map(e => e.friendid).join();
    ...
    loadCont() {
        const filePath =
            this.referenceFile ||
            path.join(__dirname, '..', 'config', 'contributors.json');
        let contents;
