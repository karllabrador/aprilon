...
cards.fetchData = () => {
  $.getJSON(cards.config.url, (data) => {
    $.each(data, (key, val) =>
      $(cards.config.selector).append(cards.buildCard(val))
    );
  });
};
...
cards.buildCard = (v) => {
  ...
  let buttons = "";
  if (v.github)
    buttons += `<a href="https://github.com/${v.github}"><img src="/img/github-mark-64px.png" width="28px" alt="Github logo" /></a>`;
  ...
  return `
        <div class="column is-one-third">
            <div class="card">
                <div class="avatar is-pulled-left${cards.getStateClass(
                  v.steam_state
                )}">
                    <img src="${v.steam_avatar_url}" alt="${
    v.steam_name
  }'s steamcommunity.com avatar"/>
                </div>
